...
def get_debt(debt):
    debt.rename(
        columns={"Date": "date", "Loans": "loans", "Debt": "debt"}, inplace=True
    )
...
def get_investments(investments):
...
def budget_summary(transactions, budget):
    ...
    spent_avg = transactions[
        (transactions["date"] >= start_date) & (transactions["date"] <= end_date)
    ]

    ## Group by 'Budget.Head' and calculate the mean spend for each budget head
    spent_avg = (
        spent_avg.groupby(["budget_head", pd.Grouper(key="date", freq="ME")])["debit"]
        .sum()
        .reset_index()
    )
...
def spend_calc(transactions, budget):
    ...
    spend = (
        spend.groupby(pd.Grouper(key="date", freq="ME"))["debit"].sum().reset_index()
    )
    ...
    spend["Lower Bound"] = (
        spend["Moving Average"] - spend["Standard Deviation"]
    )  # *1.96
    spend["Upper Bound"] = (
        spend["Moving Average"] + spend["Standard Deviation"]
    )  # *1.96
...
def income_calc(transactions, budget):
    ...
    income_180["budget_head"] = income_180["budget_head"].replace(
...
def income_high(transactions, budget):
    ...
    return income_180.sort_values(by="credit", ascending=False).head(5)[
...
def expense_calc(transactions, budget):
    ...
    expense_180["budget_head"] = expense_180["budget_head"].replace(
        "Work Expense", "Reimbursement"
    )
...
def expense_high(transactions, budget):
    ...
    return expense_180.sort_values(by="debit", ascending=False).head(5)[
...
def lic_debit(transactions):
    return transactions[
        (transactions["description"].str.contains("ACH D- LIC OF INDIA-9115721030522"))
