...
def budget_fig_gen(transactions, budget):
    ...
    budget_fig.add_scatter(
        y=budget["budget_head"],
        x=budget["mean"],
        mode="markers",
    ...
    budget_fig.update_layout(
        title={
            "text": "Budgetted vs Spent in the current month",
            "xanchor": "center",
            "x": 0.5,
        },
        yaxis_title="Budget Head",
        xaxis_title="Budgetted/Spent",
        font={"size": 18},
    )
    ...
    budget_fig.update_layout(
        legend=dict(title_text="", orientation="h", yanchor="top", y=1, x=0.625)
    )
...
def spend_earn_fig(transactions, budget):
    ...
    # Add title and axis labels
    earn_fig.update_layout(
        title="Amount Earned vs Spent per Month",
        xaxis_title="Date",
        yaxis_title="Credit",
        title_x=0.5,
        title_font_size=15,
...
def income_fig_gen(transactions, budget):
    ...
    income_fig = px.box(
        income_180, x="budget_head", y="credit", height=450, boxmode="group", width=800
    )

    # Add title and axis labels
    income_fig.update_layout(
        title="Income Distribution across Sources (Last Six Months)",
        xaxis_title="Source",
        yaxis_title="Credit",
    )
...
def expense_fig_gen(transactions, budget):
    ...
    expense_fig = px.box(
        expense_180, y="budget_head", x="debit", height=450, boxmode="group", width=1000
    )

    # Add title and axis labels

    expense_fig.update_layout(
        title="Expense Distribution across Budget Heads (Last Six Months)",
        yaxis_title="Budget Head",
        xaxis_title="Debit",
    )
...
def instruments_fig(investments):
    ...
    investments["percent-profit"] = (
        investments["profit"] * 100 / investments["invested"]
    )

    # remove infinities in percent profit

    investments = investments.replace([np.inf, -np.inf], np.nan).dropna(
        subset=["percent-profit"], how="all"
    )

    # calculate mean and standard deviation of percent profit grouped by type
    grouped_investments = (
        investments.groupby("type")["percent-profit"].agg(["mean", "std"]).reset_index()
    )

    # plot mean and std as error bars on a bar plot
    instruments_fig = px.bar(
        grouped_investments, x="type", y="mean", error_y="std", width=750, height=450
    )

    # make consistent with other figs
    instruments_fig.update_layout(
        title={
            "text": "Mean and Standard Deviation of Percent Profit by Instrument Type",
            "xanchor": "center",
            "x": 0.5,
        },
        yaxis_title="Percent Profit",
...
def equities_fig(investments):
    ...
    equities_fig.update_layout(
        title={
            "text": "Invested vs Current Value of Equities",
            "xanchor": "center",
            "x": 0.5,
        },
        xaxis_title="Amount",
...
def mutual_funds_fig(investments):
    ...
    mutual_funds_fig.update_layout(
        title={
            "text": "Invested vs Current Value of Mutual Funds",
            "xanchor": "center",
            "x": 0.5,
        },
        xaxis_title="Amount",
...
def net_value_fig(investments):
    ...
    net_value = (
        investments.groupby(["date", "type"])["Current-value"].sum().reset_index()
    )

    # plot sum of current value by type over date

    net_value_fig = px.line(
        net_value, x="date", y="Current-value", color="type", width=750, height=450
    )
    ...
    net_value_fig.update_layout(
        title={
            "text": "Change in Net Value of Investments over Time by Type",
            "xanchor": "center",
            "x": 0.5,
        },
        xaxis_title="Date",
